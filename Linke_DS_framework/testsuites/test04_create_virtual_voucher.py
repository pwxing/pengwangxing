...
class CreateVirtualvoucher(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_linke_login(self):
        ...
        time.sleep(1)
        goodspage.click_virtual_goods_type4()
        # 输入商品名称
        goodspage.type_goods_name(u"python自动创建代金券"+goodspage.now)
        # 输入商品编码
        ...
        time.sleep(1)
        goodspage.click_menu("点击选择")
        self.driver.switch_to.frame(goodspage.voucher_frame)
        time.sleep(1)
        goodspage.click_voucher_select_btn()
        time.sleep(1)
        self.driver.switch_to_default_content()
